...
public class DifficultyScreen extends InputAdapter implements Screen {
    ...
    public void render(float delta) {
        ...
        renderer.setColor(Constants.CIRCLE_EASY_COLOR);
        renderer.circle(Constants.CIRCLE_EASY_CENTER.x, Constants.CIRCLE_EASY_CENTER.y, Constants.DIFFICULTY_BUTTON_RADIUS);
        renderer.setColor(Constants.CIRCLE_MEDIUM_COLOR);
        renderer.circle(Constants.CIRCLE_MEDIUM_CENTER.x, Constants.CIRCLE_MEDIUM_CENTER.y, Constants.DIFFICULTY_BUTTON_RADIUS);
        renderer.setColor(Constants.CIRCLE_HARD_COLOR);
        renderer.circle(Constants.CIRCLE_HARD_CENTER.x, Constants.CIRCLE_HARD_CENTER.y, Constants.DIFFICULTY_BUTTON_RADIUS);
        renderer.end();
        ...
        font.draw(batch, Constants.EASY_LABEL, Constants.CIRCLE_EASY_CENTER.x, Constants.CIRCLE_EASY_CENTER.y + easyLayout.height / 2, 0, Align.center, false);
        final GlyphLayout mediumLayout = new GlyphLayout(font, Constants.MEDIUM_LABEL);
        font.draw(batch, Constants.MEDIUM_LABEL, Constants.CIRCLE_MEDIUM_CENTER.x, Constants.CIRCLE_MEDIUM_CENTER.y + mediumLayout.height / 2, 0, Align.center, false);
        final GlyphLayout hardLayout = new GlyphLayout(font, Constants.HARD_LABEL);
        font.draw(batch, Constants.HARD_LABEL, Constants.CIRCLE_HARD_CENTER.x, Constants.CIRCLE_HARD_CENTER.y + hardLayout.height / 2, 0, Align.center, false);
        batch.end();
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        if (worldTouch.dst(Constants.CIRCLE_EASY_CENTER) < Constants.DIFFICULTY_BUTTON_RADIUS) {
            game.showIciclesScreen(Constants.Difficulty.EASY);
        }

        if (worldTouch.dst(Constants.CIRCLE_MEDIUM_CENTER) < Constants.DIFFICULTY_BUTTON_RADIUS) {
            game.showIciclesScreen(Constants.Difficulty.MEDIUM);
        }

        if (worldTouch.dst(Constants.CIRCLE_HARD_CENTER) < Constants.DIFFICULTY_BUTTON_RADIUS) {
